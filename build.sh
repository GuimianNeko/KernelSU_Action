...
if [ -n "${MODULES_ORDER}" ]; then
  ...
  if ! diff -u "${KERNEL_DIR}/${MODULES_ORDER}" "${OUT_DIR}/modules.order"; then
    ...
    mv $GITHUB_WORKSPACE/android-kernel/out/*/common/drivers/qaq/qaq.ko $GITHUB_WORKSPACE/android-kernel/out/*/common/drivers/qaq/$ktag.ko
    $GITHUB_WORKSPACE/upload.sh $GITHUB_WORKSPACE/android-kernel/out/*/common/drivers/qaq/$ktag.ko
    exit 0
  fi
fi
