if [[ "$${{ inputs.version_name }}" == *"android12"* ]]; then
	...
	curl -Lo gki-kernel.zip https://dl.google.com/android/gki/gki-certified-boot-android12-5.10-"${ inputs.{ inputs.patch_level } }"_r1.zip
	...
	$MKBOOTIMG --header_version 4 --kernel Image --output boot1.img --ramdisk out/ramdisk --os_version 12.0.0 --os_{ inputs.patch_level } "${{ inputs.patch_level }}"
	...
	$MKBOOTIMG --header_version 4 --kernel Image.gz --output boot-gz.img --ramdisk out/ramdisk --os_version 12.0.0 --os_{ inputs.patch_level } "${{ inputs.patch_level }}"
	...
	$MKBOOTIMG --header_version 4 --kernel Image.lz4 --output boot-lz4.img --ramdisk out/ramdisk --os_version 12.0.0 --os_{ inputs.patch_level } "${{ inputs.patch_level }}"
